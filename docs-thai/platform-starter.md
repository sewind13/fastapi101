...
runtime dependency ของ extension หลายตัวถูกแยกเป็น optional extras เพื่อให้ production image พื้นฐานไม่แบก dependency ที่ยังไม่ใช้ เช่น Redis-backed cache/rate limit ใช้ Redis extra อย่าง `<your-package>[redis]`, SES/S3 ใช้ `<your-package>[aws]`, และ OpenTelemetry exporter ใช้ `<your-package>[observability]`
...
ถ้าเปิด AMQP worker, outbox dispatcher, หรือ DLQ replay ให้ติดตั้ง worker extra เช่น `<your-package>[worker]`

## Suggested adoption path

ถ้าต้องการ preset ที่ copy ไปใช้ได้เร็ว ให้ดู `core-only`, `redis-enabled`, และ `full-async` ใน [adoption-checklists.md](../docs-thai/adoption-checklists.md)
...
- ถ้าไม่แน่ใจ เริ่มจาก `Core` แล้วค่อยไล่ดู [adoption-checklists.md](../docs-thai/adoption-checklists.md)
...
อ่านฉบับอังกฤษละเอียดได้ที่ [docs/platform-starter.md](../docs/platform-starter.md)
